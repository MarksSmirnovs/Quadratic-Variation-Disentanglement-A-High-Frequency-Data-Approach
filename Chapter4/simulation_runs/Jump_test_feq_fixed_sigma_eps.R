...
#C_grid=data.frame(C=c(rep(1/19,12),rep(1/18,4)))

Paramater_sets = expand.grid(sigma_J = sigma_J_grid,
                             n = n_grid
)
...
sim_result = pbapply::pblapply(cl = cluster, X = 1:nrow(Paramater_sets), function(params_idx){
  ...
  rownames(return_single_sim) <- paste("sigma_J = ", Paramater_sets[params_idx,]$sigma_J,
                                       "|n_sampl= ",  Paramater_sets[params_idx,]$n,
                                       sep = "")
...
saveRDS(sim_result,"sim_result_jumptest_n_J.rds")
