...
Paramater_sets = expand.grid(sigma_eps = sigma_eps_grid, n_freq = n_grid)
...
sim_result = lapply( X = 1:nrow(Paramater_sets), function(params_idx){
  ...
  clusterExport(cl = cluster, c("sigma_eps", "n_sampl"), environment())
  ...
  total <- try(readRDS("BPV_preAvg_Sigma_eps_n_freq.rds"))
  if(class(total) == "try-error"){
    saveRDS(return_single_sim, "BPV_preAvg_Sigma_eps_n_freq.rds")
  }else{
    saveRDS(rbind(total, return_single_sim), "BPV_preAvg_Sigma_eps_n_freq.rds")
  }
  return_single_sim
}) %>% do.call(what = rbind)
stopCluster(cluster)
...
saveRDS(sim_result, file = "BPV_preAvg_Sigma_eps_n_freq_total.rds")