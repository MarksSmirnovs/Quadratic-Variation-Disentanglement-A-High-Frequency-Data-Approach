...
sim1 <- readRDS("SimulationStudy/results/bpv_adj_sim1.rds")
SimData <- readRDS("SimulationStudy/results/BPV_adj_Sigma_eps_Sigma_J_noBoot.rds")
...
SimData <- SimData %>% arrange(sub(".*\\|", "\\1", Parameters))
...
for(i in 1:nrow(SimData)){
  ...
  sigma_eps %<>% c(paste0("$\\sigma_\\varepsilon = ", sub("*\\|.*", "\\1",  Params[[1]][1]), "$"))
  jump_stds %<>% c(paste0("\\rotatebox{90}{\\tiny $\\sigma_J = ", sub(".*\\|", "\\1", Params[[1]][2]), "$}"))
