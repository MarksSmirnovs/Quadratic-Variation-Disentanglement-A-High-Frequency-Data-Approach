...
SimData <- readRDS("SimulationStudy/results/QV_sim_BPV_preAvg.rds")

SimData <- SimData[c("Kernel", "TSRV", "PreAvg"),]

#Transpose data
SimData = t(SimData) %>% as.data.frame() %>% round(3) %>% rownames_to_column("Parameters")

SimData <- SimData %>% filter(grepl(pattern = "sigma_J = 0.00075", x = Parameters) | 
                     grepl(pattern = "sigma_J = 0.00115", x = Parameters) | 
                     grepl(pattern = "sigma_J = 0.0015", x = Parameters))
...
colnames(df)[1:2] <- ""
colnames(df) <- gsub(pattern = "PreAvg", replacement = "$\\JV\\^{PRE}$", x = colnames(df))
colnames(df) <- gsub(pattern = "Kernel", replacement = "$\\JV\\^{Kernel}$", x = colnames(df))
colnames(df) <- gsub(pattern = "TSRV", replacement = "$\\JV\\^{TSRV}$", x = colnames(df))
