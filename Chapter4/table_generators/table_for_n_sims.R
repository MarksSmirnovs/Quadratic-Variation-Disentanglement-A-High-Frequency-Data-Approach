...
Data = readRDS("SimulationStudy/results/sim_study_noise_n.rds")
...
TableGenerator = function(SimData){
  ...
  for(i in 1:(nrow(SimData))) {
    ...
    Red_int = abs(df[2*i,-c(1, TSRV_idx)] %>% gsub("[(]","",.) %>% gsub("[)]","",.) %>% as.numeric()-0.95) %>% which.min()
    
    df[2*i,Red_int+2] = paste0( "\\textcolor{red}{",df[2*i,Red_int+2] ,"}")
    df[2*i,TSRV_idx] = paste0( "\\st{", df[2*i,TSRV_idx] ,"}")
  }
  ...
  for(i in 1:nrow(SimData)){
    ...
    sigma_eps %<>% c(paste0(gsub("n_sampl","$n", Params[[1]][1], fixed = TRUE),"$"))
    jump_stds %<>% c( paste0("\\rotatebox{90}{",gsub("sigma_eps","$\\sigma_\\varepsilon", Params[[1]][2], fixed = T),"$}"))
...
Data = readRDS("SimulationStudy/results/sim_study_sigmaJ_n.rds")

TableGenerator_simga_j = function(SimData){
  ...
  for(i in 1:nrow(SimData)){
    ...
    sigma_eps %<>% c(paste0(gsub("n_sampl","$n", Params[[1]][1], fixed = TRUE),"$"))
    jump_stds %<>% c( paste0("\\rotatebox{90}{",gsub("sigma_J ","$\\sigma_J", Params[[1]][2], fixed = T),"$}"))
...
cat(TableGenerator_simga_j(Data))
